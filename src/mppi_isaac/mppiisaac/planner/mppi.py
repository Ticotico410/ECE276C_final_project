...
class MPPIConfig(object):
    """
        ...
        :param topk_vis: Number of top trajectories to visualize in rviz
    """
...
class MPPIPlanner(ABC):
    ...
    def _update_distribution(self, costs, actions):
        ...
        if self.update_cov:
            ...
            weighted_delta = w.unsqueeze(-1).unsqueeze(-1) * (delta ** 2)
    ...
    def get_topk_trajectories(self, topk=10):
        ...
        if self.actions is None or self.cost_total is None:
            return None
        
        # Get topk indices with lowest costs
        topk_values, topk_idx = torch.topk(self.cost_total, min(topk, self.K), largest=False)
        
        # Select topk trajectories
        topk_actions = torch.index_select(self.actions, 0, topk_idx)
