...
class IsaacGymWrapper:
    def __init__( self, cfg: IsaacGymConfig, actors: List[ActorWrapper], init_positions: List[List[float]], num_envs: int, viewer: bool = False,):
        ...
        # Live name caches built from the simulator (env 0)
        self._actor_name_to_env_index = None      # name -> root_state column (DOMAIN_ENV)
        self._actor_name_to_handle    = None      # name -> handle (env 0)
        self._link_index_cache        = {}        # (actor_name, link_name) -> rigid_body idx (DOMAIN_ENV)
        self._name_cache_env_version  = 0         # bump when we recreate sim
        self._rebuild_name_caches()               # initial build
    ...
    def update_block_state_tensor_by_obstacles(self, block_tensor):
        ...
        #print('-----------------------------------------------------------------------------------------------------')
        #print('[Info] Block position: ', block_tensor[:3])
        #print('[Info] Block orientation: ', block_tensor[3:7])
    ...
    # -------- name/idx discovery from the live simulator --------
    def _rebuild_name_caches(self):
        """Scan env 0 and build name→index/handle maps from Gym (true source of truth)."""
        env0 = self.envs[0]
        actor_count = self.gym.get_actor_count(env0)

        actor_name_to_env_index = {}
        actor_name_to_handle    = {}

        for i in range(actor_count):
            h = self.gym.get_actor_handle(env0, i)
            name = self.gym.get_actor_name(env0, h)  # the string you passed to create_actor(...)
            # Map to the DOMAIN_ENV index (column in root_state)
            env_idx = self.gym.get_actor_index(env0, h, gymapi.IndexDomain.DOMAIN_ENV)
            actor_name_to_env_index[name] = env_idx
            actor_name_to_handle[name] = h

        self._actor_name_to_env_index = actor_name_to_env_index
        self._actor_name_to_handle    = actor_name_to_handle
        self._link_index_cache.clear()  # links depend on handles; clear when actors change

    def _ensure_name_caches(self):
        """Ensure caches exist; if missing a name, rebuild once."""
        if self._actor_name_to_env_index is None or self._actor_name_to_handle is None:
            self._rebuild_name_caches()

    def get_actor_index(self, actor_name: str) -> int:
        """
        Return the DOMAIN_ENV actor index (column in sim.root_state) for 'actor_name'.
        Rebuilds caches on-demand if the name isn't found yet (e.g., actor added later).
        """
        self._ensure_name_caches()
        idx = self._actor_name_to_env_index.get(actor_name, None)
        if idx is None:
            # maybe a new actor was added after caches were built; rebuild once
            self._rebuild_name_caches()
            idx = self._actor_name_to_env_index.get(actor_name, None)
        if idx is None:
            known = list(self._actor_name_to_env_index.keys())
            raise RuntimeError(f"Actor '{actor_name}' not found in sim. Known: {known}")
        return idx

    def get_actor_index_tensor(self, actor_name: str, device="cuda:0") -> torch.Tensor:
        """Same as get_actor_index, but returns a 0-D CUDA tensor for torch.index_select."""
        return torch.tensor(self.get_actor_index(actor_name), device=device, dtype=torch.long)

    def get_link_index(self, actor_name: str, link_name: str) -> int:
        """
        Return the rigid-body index (row in sim.rigid_body_state) for link_name on actor_name,
        using DOMAIN_ENV indexing.
        """
        key = (actor_name, link_name)
        cached = self._link_index_cache.get(key, None)
        if cached is not None:
            return cached

        self._ensure_name_caches()
        h = self._actor_name_to_handle.get(actor_name, None)
        if h is None:
            # late-added actor? rebuild once
            self._rebuild_name_caches()
            h = self._actor_name_to_handle.get(actor_name, None)
        if h is None:
            known = list(self._actor_name_to_handle.keys())
            raise RuntimeError(f"Actor '{actor_name}' not found when resolving link '{link_name}'. Known: {known}")

        rb_idx = self.gym.find_actor_rigid_body_index(
            self.envs[0], h, link_name, gymapi.IndexDomain.DOMAIN_ENV
        )
        if rb_idx < 0:
            raise RuntimeError(f"Link '{link_name}' not found on actor '{actor_name}'.")
        self._link_index_cache[key] = rb_idx
        return rb_idx

    def get_link_index_tensor(self, actor_name: str, link_name: str, device="cuda:0") -> torch.Tensor:
        """Same as get_link_index, but returns a 0-D CUDA tensor for direct torch indexing."""
        return torch.tensor(self.get_link_index(actor_name, link_name), device=device, dtype=torch.long)